...
class StepsReader {
    ...
    async annotationSteps(metadata)
    {
        ...
        for( let code of Object.keys(metadata) )
        {
            ...
            if( file )
            {
               steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.file( code, file, user, persistent, target )));
            }
            else if( serve )
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.serve( code, user, persistent, target )));
            }
            else if( expect )
            {
            ...
            {
                steps.push( async ($,sl) => this._setResults(this.codeBlockSelector($, metadata[code].codeIndex), await sl.run( code, user, persistent, target )) );
            }
        }
        return steps;
    }
            ...
            if( step instanceof Object )
            {
                ...
                switch( key )
                {
                    case "file":
                    {
                        ...
                        steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.file( sl.asContainsNext($, searchText), file, user, persistent, target, permission )));
                        break;
                    }
                    case "serve":
                    {
                        let searchText = step[key];
                        steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.serve( sl.asContainsNext($, searchText), user, persistent, target )));
                        break;
                    }
                    ...
                    case "run":
                    {
                        ...
                        else
                        {
                            ...
                            steps.push( async ($,sl) => this._setResults(() => sl.asContainsNextElement($, searchText), await sl.run( sl.asContainsNext($, searchText), user, persistent, target, opts )) );
                            break;
                        }
                    }
                    default:
