...
class Select {
    ...
    async file(content, destination, user, persistent, target, permission)
    {
    ...
    async run(content, user, persistent, target, opts = { prompts: undefined })
    {
        // opts.prompts = [
    ...
    async serve(content, user, persistent, target)
    {
        return new Promise(async(resolve, reject) => 
        {
            let cmd = tr.trimPrompt(content);
